...
class QP:
    ...
    def gradient(self, X, Y):
        ...
        phi = np.expand_dims(self.phi,axis=1)
        grad_theta = np.mean((X@(theta**2-phi**2)-Y[:,np.newaxis])*(X*theta.T),axis = 0)
        grad_phi = -np.mean((X@(theta**2-phi**2)-Y[:,np.newaxis])*(X*phi.T),axis = 0)
