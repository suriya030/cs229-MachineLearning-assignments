...
class DecisionTree:
    ...
    def _misclassification_loss(self, y):
        ...
        loss = None
        # *** START CODE HERE ***
        p_max = np.max(np.bincount(y))/ len(y)
        loss = 1 - p_max
        # *** END YOUR CODE ***
    ...
    def _best_split(self, X, y):
        ...
        for feature_idx in range( n_features ) :
            cutoff_values = np.unique( X[:, feature_idx])
            for cutoff in cutoff_values:
                r_bool = X[: ,feature_idx] > cutoff
                l_bool = X[:,feature_idx] <= cutoff
                if np.sum( l_bool) == 0:
                    continue
                if np.sum(r_bool )==0:
                    continue
                totLoss = self._misclassification_loss( y[l_bool])*np.sum(l_bool )+self._misclassification_loss(y[r_bool ])*np.sum(r_bool)

                if totLoss < min_loss:
                    best_idx,best_thr = feature_idx, cutoff
                    min_loss = totLoss
        # *** END YOUR CODE ***
    ...
    def _grow_tree(self, X, y, depth=0):
        ...
        if depth < self.max_depth:
            ...
            if best_idx is not None:
                ...
                r_bool = X[: ,best_idx] > best_thr
                l_bool = X[:,best_idx] <= best_thr
                root_node.right= self._grow_tree(X[r_bool] , y[r_bool],depth +1) 
                root_node.left = self._grow_tree( X[l_bool ], y[ l_bool] , depth+ 1)      
